...
#      		daylireport of Al : www.virtu-al.net
...
########################################################################################### 
#Derivative from Ph Koenig
# use convertto-html parameters
# And generate the page at last time
###########################################################################################
##Version
##############################
...
##fixed definition
$pkoversion = "TiddlyEsxDoc 1.6 Unclephil-200910"
$crlf="`n"
...
if ($CFG -eq ""){
    write-host "No parameter provided`nUsing default ./TWEDConfig.ps1 parameters"
	$CFG = "./TWEDConfig.ps1"
}
if (Test-Path $CFG){
	Import-Module $CFG
	...
	Write-Host "TiddlyEsxDoc Stopping"
	break
}
...
Connect-VIServer $vcserver -port $portvc -User $uc -Password $pw
 ...
 foreach ($vmhost in $vmhosts){
	...
	foreach($pnic in $networkSystem.NetworkConfig.Pnic){
		...
		foreach($Hint in $pnicInfo){
			...
			$ct += "*VLAN" + $crlf + $NetworkInfo.VLAN +$crlf
		}
	}
...
ForEach ($vm in $vms)
{
	...
	for  ($i=0; $i -le 9;$i++){
		if ($vm.guest.ipAddress[$i]){$ct += "** Ip: " + $vm.guest.ipAddress[$i] + $crlf}
    	}
